...
class RegisterView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        frontend = 'http://localhost:3000/verify-email'

        relativeLink=reverse('email-verify')
        # absurl='http://'+ current_site+relativeLink+"?token="+str(token)
        absurl=frontend+"?token="+str(token)
...
class RequestPasswordResetEmail(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        if User.objects.filter(email=email).exists():
            ...
            # relativeLink = reverse(
            #     'localhost:3000/reset-confirm', kwargs={'uidb64': uidb64, 'token': token})
            frontend = 'localhost:3000/reset-confirm'
            # absurl = 'http://'+current_site + relativeLink
            absurl = 'http://'+frontend+"/"+uidb64+"/"+token
